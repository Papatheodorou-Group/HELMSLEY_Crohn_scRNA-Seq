...
python ${SCRIPT_DIR}/subset_obs.py "${IN_OBJ}" "${OBS}" "${OUT_OBJ}"

# reference
python ${SCRIPT_DIR}/print_obs.py "${OUT_OBJ}" "${STAT_PREFIX}.tsv"
Rscript ${SCRIPT_DIR}/stats_barplot.R "${STAT_PREFIX}.tsv" "${STAT_PREFIX}"

python ${SCRIPT_DIR}/create_scVI_model.py "${OUT_OBJ}" "${MOD}" "${OUT_OBJ}" ${REDUCT_NAME}
